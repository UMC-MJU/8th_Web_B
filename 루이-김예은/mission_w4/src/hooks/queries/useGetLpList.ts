...
function useGetLpList({ cursor, search, order, limit }: PaginationDto) {
  return useQuery({
    ...
    // 조건에 따라 쿼리를 실행 여부 제어, true가 기본값
    //enabled: Boolean(search), //검색어가 있는 경우 true,

    //refetchInterval: 10 * 60, // 랭킹 데이터가 10초마다 업데이트

    // retry: 쿼리 요청이 실패했을 때 자동으로 잿도할 횟수를 지정합니다.
    //기본값은 3회 정도, 네트워크 오류 등 임시적인 문제를 보완할 수 있습니다.

    // initialData: 쿼리 실행 전 미리 제공할 초기 데이터를 설정합니다.
    // 컴포넌트 렌더링 될 때 빈 데이터 구조를 미리 제공해서, 로딩 전에도 안전하게 UI를 구성할 수 있게 해준다.
    // initialData: initialLpListData,

    //파라메터가 변경될 때 이전 데이터를 유지하여 UI 깜빡임(Flicking)을 줄여줍니다.
