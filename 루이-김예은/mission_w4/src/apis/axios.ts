...
export const axiosInstance = axios.create({
  ...
  //withCredentials: true, // 쿠키가 모든 요청에 심김김
});

// 요청 인터셉터: 모든 요청 전에 accessToken을 Authorization 헤더에 추가한다.
axiosInstance.interceptors.request.use(
  (config) => {
    const { getItem } = useLocalStotrage(LOCAL_STORAGE_KEY.accessToken);
...
// 응답 인터셉터: 401 에러 발생 -> refresh 토큰을 통한 갱신을 처리한다.
axiosInstance.interceptors.response.use(
  (response) => response, // 정상 응답 그대로 반환
  async (error) => {
    const originalRequest: CustomInternalAxiosRequestConfig = error.config;

    // 401 에러면서, 아직도 재시도 하지 않은 요청 경우 처리
    if (
      error.response &&
      ...
      if (originalRequest.url === "/v1/auth/refresh") {
        const { removeItem: removeAccessToken } = useLocalStotrage(
          LOCAL_STORAGE_KEY.accessToken
        );
        const { removeItem: removeRefreshToken } = useLocalStotrage(
      ...
      if (!refreshPromise) {
        ...
        refreshPromise = (async () => {
          const { getItem: getRefreshToken } = useLocalStotrage(
            LOCAL_STORAGE_KEY.refreshToken
          );
      ...
      return refreshPromise.then((newAccessToken) => {
        //원본 요청의 Authorization 헤더를 갱신된 토큰으로 업뎃뎃
