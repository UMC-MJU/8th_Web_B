...
interface CustomInternalAxiosRequestConfig extends InternalAxiosRequestConfig {
  _retry?: boolean; //요청 재시도여부를 나타내는 플래그
}
...
export const axiosInstance = axios.create({
  ...
  // headerimport { axiosInstance } from './axios';
...
axiosInstance.interceptors.request.use(
  (config) => {
    ...
    //accessToken이 존재하면 Authorization 헤더에 Bearer 토큰 방식으로 추가한다.
    if (accessToken) {
...
//응답 인터셉트 401에러 발생 => RefreshToken을 통한 토큰 갱신 처리
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      //Refresh 엔드포인트 401에러가 발생한 경우 (UnAuthoriztion), 중복 재시도 방지를 위해 로그아웃 처리
