...
const SignupPage = () => {
  ...
  // 단계 전환 핸들러
  const handleNext = async (field: keyof FormFields | (keyof FormFields)[]) => {
        ...
        {step === 2 && (
          ...
          >
            ...
            <button
              type="submit"
              disabled={
                watch("password") !== watch("passwordCheck") || isSubmitting
              }
        ...
        {step === 3 && (
          ...
          >
            <input
              {...register("name")}
              className={`border border-[#ccc] w-[300px] p-[10px] focus:border-[#807bff] rounded-lg ${
                errors?.name ? "border-blue-300 bg-gray-900" : "border-gary-300"
              }`}
              type={"name"}
              placeholder={"이름"}
            />
